   ...
   class PatrolAndReactTree : ParallelTask
    {
        public PatrolAndReactTree(Monster character, GameObject target, Vector3 position1, Vector3 position2) {
            ...
            Interrupter PatrolBehavior = new Interrupter(new Inverter(new UntilFail(new PatrolTree(character, position1, position2))));
            UntilFail ReactBehavior = new UntilFail(new Inverter(new ReactTree(character, target, PatrolBehavior)));
